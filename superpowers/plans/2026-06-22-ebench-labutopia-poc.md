...
| Render/readback evidence | Blocked | 2026-06-23 diagnostics show all three tasks are `readback_black_before_recorder`; recorder writing is not the root cause. |
...
- Regression tests: `python -m pytest tests/labutopia_poc -q` -> `25 passed, 1 skipped`
...
1. P0 render source fix: fix camera axes/pose handling and deterministic lighting until `camera2` readback is non-black in the normal eval path.
