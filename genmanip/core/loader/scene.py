...
from genmanip.core.loader.preload_rules import (
    apply_rule,
    collect_all_colors,
...
from genmanip.core.loader.robot import (
...
from genmanip.core.loader.utils import reset_object_xyz, collect_world_pose_list
from genmanip.utils.pointcloud.pointcloud import (
    objectList2meshList,
    get_mesh_info_by_load,
)
from genmanip.core.embodiment import (
    FrankaNormalEmbodiment,
    FrankaRobotiqEmbodiment,
    AlohaSplitEmbodiment,
    Lift2Embodiment,
)
from genmanip.core.embodiment import BaseEmbodiment  # type: ignore
...
from genmanip.utils.planner.curobo.utils import get_curobo_planner, CuroboPlanner
...
def load_object_pool(scene: dict, demogen_config: dict, current_dir: str) -> None:
    if "object_data_path" in demogen_config["domain_randomization"]:
        ...
        scene["object_pool"] = ObjectPool(
            os.path.join(current_dir, "assets/objects/objaverse_annotation_refined_pnp.pickle")
        )
...
def build_scene_from_config(
    ...
    if (
        ...
        scene["tcp_configs"]["franka"] = load_yaml(
            os.path.join(current_dir, "configs/robots/tcp/franka_robotiq_tcp.yaml")
        )
    else:
        scene["tcp_configs"]["franka"] = load_yaml(
            os.path.join(current_dir, "configs/robots/tcp/franka_tcp.yaml")
        )
